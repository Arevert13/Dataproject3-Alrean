...
resource "null_resource" "package_lambdas" {
  triggers = {
    get_product_hash = filemd5("${var.lambda_dir}/get_product.zip")
    add_product_hash = filemd5("${var.lambda_dir}/add_product.zip")
    buy_product_hash = filemd5("${var.lambda_dir}/buy_product.zip")
  }
}
locals {
  lambda_env = {
...
resource "aws_lambda_function" "get_product" {
  ...
  source_code_hash = filebase64sha256("${var.lambda_dir}/get_product.zip")
  filename         = "${var.lambda_dir}/get_product.zip"
...
resource "aws_lambda_function" "add_product" {
  ...
  runtime          = "python3.11"
  role             = aws_iam_role.lambda_exec_role.arn
  source_code_hash = filebase64sha256("${var.lambda_dir}/add_product.zip")
  filename         = "${var.lambda_dir}/add_product.zip"
...
resource "aws_lambda_function" "buy_product" {
  ...
  runtime          = "python3.11"
  role             = aws_iam_role.lambda_exec_role.arn
  source_code_hash = filebase64sha256("${var.lambda_dir}/buy_product.zip")
  filename         = "${var.lambda_dir}/buy_product.zip"
